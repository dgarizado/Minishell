...
/*   Updated: 2023/06/07 19:33:11 by dgarizad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_expand(int i, int j, int flag_expand, int flag_quote)
{
	...
	while (g_data.token1[i] != NULL)
	{
		...
		{	
			...
			if (g_data.token1[i][j] == '$' && g_data.token1[i][j + 1] != '\0' && g_data.token1[i][j + 1] != ' ') //THIS FIXES THE PROBLEM OF $ AT THE END OF THE STRING
			{
				ft_expand(i, j);
				flag_quote = 0;
				flag_expand = 1;
